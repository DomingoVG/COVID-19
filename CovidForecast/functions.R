...
recLag <- function(infections, deaths, datCols = dateCols(infections), ttr = 14, active = TRUE){
  matI<-as.matrix(infections[, datCols])
  matD<-as.matrix(deaths[, datCols])
  matA<-matI-matD #remove deaths
  if (length(ttr)==1){
    matR <- cbind(matrix(0, nrow = nrow(matA), ncol = 14), matA[, -((ncol(matA)-13):ncol(matA)), drop = FALSE]) # "recovered"
  } else {
...
recLagOptim <- function(infections, deaths, recoveries){
  ...
  out <- round(optimize(f = errorFunc, interval = c(10, 24), infections=infections, deaths=deaths, recoveries=recoveries)$minimum, 0) # get the optimum
  if (out==24) out <- 15 # catch cases where optimize has crashed
  out
}
